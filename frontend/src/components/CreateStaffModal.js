...
// Main Component
const CreateStaffModal = ({ isOpen, onClose, onStaffCreated, showToast }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.password.trim()) {
      showToast('Password is required', 'error', 'Validation Error', 3000);
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        showToast(response.data.message || 'Staff created successfully!', 'success', 'Success', 3000);
        onStaffCreated(response.data.data);
        onClose();
      ...
      showToast('Failed to create staff', 'error', 'Error', 3000);
    } finally {
      setLoading(false);
    }
  };
        ...
        <ModalContentDiv>
          ...
          <FormSection>
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <FormGroup>
                <Label>
                  NIC <RequiredIndicator>*</RequiredIndicator>
                </Label>
                <Input
                  type="text"
                  name="nic"
